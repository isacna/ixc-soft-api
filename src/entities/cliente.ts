  ...
  ops: {
    list: op({
      ...
      toBody: (v) => {
        ...
        if (v.ativo) filters.push({ field: "cliente.ativo", value: v.ativo });
        if (v.razao) filters.push({ field: "cliente.razao", value: v.razao, operator: "L" });
        if (v.cnpjCpf) filters.push({ field: "cliente.cnpj_cpf", value: v.cnpjCpf });
        const body: Record<string, unknown> = { rp: String(v.rp ?? 50) };
